...
def create_app() -> Flask:
    ...
    def api_login():
        try:
            ...
            # #75fixam: 10 分钟内同用户名密码重登 ≤3 次
            rate_err = login_rate_limit.guard_login(username)
            if rate_err:
                return jsonify(rate_err), 200
    ...
    # -----------------------------------------------------------------------
    # Multi-account cards (Path B per-account) — see web/accounts_routes.py
    # -----------------------------------------------------------------------
    from web.accounts_routes import register_account_routes
